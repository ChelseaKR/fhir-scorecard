...
Runs :func:`fhir_scorecard.ai.narrate.narrate` over the published scorecards
...
from fhir_scorecard.ai.narrate import PROMPT_VERSION, NarrationError, narrate
from fhir_scorecard.ai.provider import Provider, ProviderError, provider_from_env


def load_records(path: Path, *, limit: int = 0) -> list[dict[str, Any]]:
    """The scorecards in a published ``scorecards.json`` dataset."""
    payload = json.loads(path.read_text(encoding="utf-8"))
    records = payload.get("scorecards", []) if isinstance(payload, dict) else []
    if not records:
        raise ValueError(f"no scorecards in {path}")
...
def score_record(
    ...
    return {
        "endpoint_id": narration.endpoint_id,
        "name": narration.name,
...
def main(argv: Sequence[str] | None = None) -> int:  # pragma: no cover - CLI wrapper
    ...
    parser.add_argument("--records", type=Path, required=True, help="published scorecards.json")
