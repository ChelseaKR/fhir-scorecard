...
class Settings:
    ...
    def from_environ(cls, environ: Mapping[str, str] | None = None) -> Settings:
        ...
        provider = env.get("FHIR_AI_PROVIDER", "anthropic").strip().lower()
        if provider not in PROVIDERS:
            raise ProviderError(f"FHIR_AI_PROVIDER must be one of {', '.join(PROVIDERS)}")
        default = DEFAULT_ANTHROPIC_MODEL if provider == "anthropic" else DEFAULT_BEDROCK_MODEL
        model = env.get("FHIR_AI_MODEL", "").strip() or default
        region = (
            env.get("FHIR_AI_AWS_REGION", "").strip()
